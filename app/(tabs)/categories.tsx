...
export default function CategoriesScreen() {
  ...
  const { categories, fiches, isLoading, initializeApp } = useStore();
  ...
  const getFicheCountForCategory = (categoryId: string) => {
    return fiches.filter(f => f.category_ids.includes(categoryId)).length;
  };
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        {categories.length === 0 ? (
          <EmptyState
            ...
            message="Les cat\u00e9gories apparaîtront ici"
          />
        ) : (
          categories.map((category) => (
            <CategoryCard
              key={category.id}
              item={category}
              count={getFicheCountForCategory(category.id)}
